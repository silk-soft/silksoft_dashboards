odoo.define('silksoft_dashboards.Dashboard', function (require) {
"use strict";
...
var DynamicDashboard = AbstractAction.extend({
    ...
    start: function() {
        ...
        return this._super().then(function() {
            session.user_has_group('silksoft_dashboards.group_dashboard_superuser_view').then(function(has_group) {
                if(has_group) {
                    $('.main-container-top').prepend('<button class="btn btn-primary add_block" data-type="tile" type="button">Add Block</button><button class="btn btn-primary add_block" style="margin-left: 10px;"data-type="graph" type="button">Add Graph</button>');
    ...
    render_dashboards: function() {
        ...
        session.user_has_group('silksoft_dashboards.group_dashboard_superuser_view').then(function(has_group) {
            if(has_group) {
                self.$('.tile-container__setting-icon').prepend('<i class="fa fa-cog block_setting fa-2x" style="cursor: pointer;"/>');
                self.$('.chart-settings-icon').prepend('<i title="Configuration" class="fa fa-cog block_setting fa-2x" style="cursor: pointer;"/>');
            }
        });
    ...
    _onClick_add_block : function(e){
        ...
        session.user_has_group('silksoft_dashboards.group_dashboard_superuser_view').then(function(has_group) {
            if(has_group) {
                ...
                ajax.jsonRpc('/create/tile', 'call', {'type' : type, 'action_id' : self.action_id }).then(function (result) {
                    if(result['type'] == 'tile'){
    ...
    _onClick_tile : function(e){
        ...
        ajax.jsonRpc('/tile/details', 'call', {
           'id': id
                ...
                if (result['model_name'] && result['model']){
                    self.do_action({
